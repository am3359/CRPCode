...
void UART1_Init(u32 bound){
   //GPIO端口设置
    ...
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;             //上拉(很重要)
...
//接收以0x0D 0x0A 结尾的命令，长度小于63，连续接收不超过4组命令
        ...
        if((COM_RX_STA & 0x8000)==0)                         //接收未完成
        {
            ...
            {    
                ...
                {//不是0x0D保持数据
                    ...
                    COM_RX_STA++;
                    if((COM_RX_STA & 0X3FFF)>(COM_REC_LEN-1))
                    {//接收数据错误,重新开始接收
                        COM_RX_STA=0;    
                        memset(COM_RX_BUF,0,COM_REC_LEN);    //清除数据接收缓冲区COM_RX_BUF,用于下一次数据接收
                    }
...
//接收以0x5A 0xA5 开始的命令，长度小于15，连续接收不超过16组命令
void USART3_IRQHandler(void)                                 //串口3中断服务程序
{//以0x5A,0xA5开头
    u8 Res;
    static u8 Hmi_rx_len;
    BaseType_t xHigherPriorityTaskWoken;

    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)    //接收中断(接收到的数据必须是0x5A,0xA5开头)
    {
        Res =USART_ReceiveData(USART3);//(USART3->DR);       //读取接收到的数据

        switch(HMI_RX_STA & 0xc000) {
            case 0x0000://没有收到0x5A,0xA5
                if(Res==0x5A)
                {//收到0x5A
                    HMI_RX_STA=0x4000;
                }
                break;
            case 0x4000://没有收到0xA5
                if(Res==0xA5)
                {//收到0x5A,0xA5
                    HMI_RX_STA=0xc000;
                }
            ...
            case 0x8000://收数据
                ...
                if((HMI_RX_STA & 0X3FFF)>=Hmi_rx_len)  //
                {
                    HMI_RX_STA=0x0000;//接收数据完成,重新开始接收
                    if(Hmi_Queue!=NULL)
                    {
            ...
            case 0xc000://收数据长度
                if (Res==0) {//长度为0，无效
                    ...
                    if (Res < HMI_REC_LEN) Hmi_rx_len = Res;
                    else Hmi_rx_len = HMI_REC_LEN - 1;
                    HMI_RX_STA = 0x8000;
                    memset(HMI_RX_BUF,0,HMI_REC_LEN);//清除数据接收缓冲区HMI_RX_BUF,用于数据接收
                }
                break;
        }
    }
//5A A5 04 81 00 01 71   读取版本                    5A A5 03 81 00 01     读取版本命令
//5A A5 13 81 20 10 18 02 08 04 11 04 33 返回RTC     5A A5 03 81 20 07     读取RTC
}
